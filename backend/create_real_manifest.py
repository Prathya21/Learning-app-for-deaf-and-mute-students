...
for video_file in sorted(videos_dir.glob('*.mp4')):
    ...
    manifest.append({
        ...
        'relative_path': 'real_videos_canonical/' + video_file.name,
...
print('Manifest written to', manifest_path, 'with', len(manifest), 'entries')

json_path = Path('data/real_videos_canonical/manifest.json')
with open(json_path, 'w') as f:
    json.dump(manifest, f, indent=2)

print('JSON manifest written to', json_path)